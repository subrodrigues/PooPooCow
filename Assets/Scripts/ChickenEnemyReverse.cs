...
public class ChickenEnemyReverse : MonoBehaviour {
public Transform target;
	...
	void Start ()
	{
		...
		target = GameObject.FindGameObjectWithTag ("ChickenEndPointReverse").transform;
	...
	void OnTriggerEnter2D (Collider2D other)
	{
		if (other.tag == "Player") {
			...
			//	Destroy (gameObject);
		} else if (other.tag == "EndPointReverse") {
			Debug.Log ("DESTROY");
			Destroy (gameObject);
