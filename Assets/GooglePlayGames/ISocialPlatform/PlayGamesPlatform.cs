...
public class PlayGamesPlatform : ISocialPlatform {
    ...
    public static void InitializeInstance(PlayGamesClientConfiguration configuration) {
        if (sInstance != null) {
            GPLogger.w("PlayGamesPlatform already initialized. Ignoring this call.");
            return;
        }
    ...
    public static PlayGamesPlatform Instance {
        get {
            if (sInstance == null) {
                GPLogger.d("Instance was not initialized, using default configuration.");
    ...
    public static bool DebugLogEnabled {
        get {
            return GPLogger.DebugLogEnabled;
        }
        set {
            GPLogger.DebugLogEnabled = value;
        }
    }
    ...
    public static PlayGamesPlatform Activate() {
        GPLogger.d("Activating PlayGamesPlatform.");
        Social.Active = PlayGamesPlatform.Instance;
        GPLogger.d("PlayGamesPlatform activated: " + Social.Active);
    ...
    public void Authenticate(Action<bool> callback, bool silent) {
        ...
        if (mClient == null) {
            GPLogger.d("Creating platform-specific Play Games client.");
    ...
    public void LoadUsers(string[] userIDs, Action<IUserProfile[]> callback) {
        GPLogger.w("PlayGamesPlatform.LoadUsers is not implemented.");
        if (callback != null) {
    ...
    public string GetUserId() {
        if (!IsAuthenticated()) {
            GPLogger.e("GetUserId() can only be called after authentication.");
            return "0";
        }
    ...
    public string GetUserDisplayName() {
        if (!IsAuthenticated()) {
            GPLogger.e("GetUserDisplayName can only be called after authentication.");
            return "";
        }
    ...
    public string GetUserImageUrl() {
        if (!IsAuthenticated()) {
            GPLogger.e("GetUserImageUrl can only be called after authentication.");
            return null;
        }
    ...
    public void ReportProgress(string achievementID, double progress, Action<bool> callback) {
        if (!IsAuthenticated()) {
            GPLogger.e("ReportProgress can only be called after authentication.");
            if (callback != null) {
        ...
        GPLogger.d("ReportProgress, " + achievementID + ", " + progress);
        achievementID = MapId(achievementID);

        // if progress is 0.0, we just want to reveal it
        if (progress < 0.000001) {
            GPLogger.d("Progress 0.00 interpreted as request to reveal.");
        ...
        if (ach == null) {
            GPLogger.w("Unable to locate achievement " + achievementID);
            GPLogger.w("As a quick fix, assuming it's standard.");
            isIncremental = false;
        } else {
            isIncremental = ach.IsIncremental;
            curSteps = ach.CurrentSteps;
            totalSteps = ach.TotalSteps;
            GPLogger.d("Achievement is " + (isIncremental ? "INCREMENTAL" : "STANDARD"));
            if (isIncremental) {
                GPLogger.d("Current steps: " + curSteps + "/" + totalSteps);
            }
        }
        ...
        if (isIncremental) {
            ...
            if (progress >= 0.0 && progress <= 1.0) {
                ...
                GPLogger.w("Progress " + progress + " is less than or equal to 1. You might be trying to use values in the range of [0,1], while values are expected to be within the range [0,100]. If you are using the latter, you can safely ignore this message.");
            }
            int targetSteps = (int)((progress / 100) * totalSteps);
            int numSteps = targetSteps - curSteps;
            GPLogger.d("Target steps: " + targetSteps + ", cur steps:" + curSteps);
            GPLogger.d("Steps to increment: " + numSteps);
            if (numSteps > 0) {
                mClient.IncrementAchievement(achievementID, numSteps, callback);
            }
        } else if (progress >= 100) {
            // unlock it!
            GPLogger.d("Progress " + progress + " interpreted as UNLOCK.");
            mClient.UnlockAchievement(achievementID, callback);
        } else {
            // not enough to unlock
            GPLogger.d("Progress " + progress + " not enough to unlock non-incremental achievement.");
        }
    }
    ...
    public void IncrementAchievement(string achievementID, int steps, Action<bool> callback) {
        if (!IsAuthenticated()) {
            GPLogger.e("IncrementAchievement can only be called after authentication.");
            if (callback != null) {
                callback.Invoke(false);
            }
            return;
        ...
        // map ID, if it's in the dictionary
        GPLogger.d("IncrementAchievement: " + achievementID + ", steps " + steps);
        achievementID = MapId(achievementID);
    ...
    public void LoadAchievementDescriptions(Action<IAchievementDescription[]> callback) {
        GPLogger.w("PlayGamesPlatform.LoadAchievementDescriptions is not implemented.");
        if (callback != null) {
    ...
    public void LoadAchievements(Action<IAchievement[]> callback) {
        GPLogger.w("PlayGamesPlatform.LoadAchievements is not implemented.");
        if (callback != null) {
    ...
    public void ReportScore(long score, string board, Action<bool> callback) {
        if (!IsAuthenticated()) {
            GPLogger.e("ReportScore can only be called after authentication.");
            if (callback != null) {
                callback.Invoke(false);
            }
            return;
        ...
        GPLogger.d("ReportScore: score=" + score + ", board=" + board);
    ...
    public void LoadScores(string leaderboardID, Action<IScore[]> callback) {
        GPLogger.w("PlayGamesPlatform.LoadScores not implemented.");
        if (callback != null) {
    ...
    public ILeaderboard CreateLeaderboard() {
        GPLogger.w("PlayGamesPlatform.CreateLeaderboard not implemented. Returning null.");
        return null;
    }
    ...
    public void ShowAchievementsUI() {
        if (!IsAuthenticated()) {
            GPLogger.e("ShowAchievementsUI can only be called after authentication.");
            return;
        }

        GPLogger.d("ShowAchievementsUI");
    ...
    public void ShowLeaderboardUI() {
        if (!IsAuthenticated()) {
            GPLogger.e("ShowLeaderboardUI can only be called after authentication.");
            return;
        }
        GPLogger.d("ShowLeaderboardUI");
    ...
    public void ShowLeaderboardUI(string lbId) {
        ...
        GPLogger.d("ShowLeaderboardUI, lbId=" + lbId);
        if (lbId != null) {
            lbId = MapId(lbId);
        }
    ...
    public void SetDefaultLeaderboardForUI(string lbid) {
        GPLogger.d("SetDefaultLeaderboardForUI: " + lbid);
        if (lbid != null) {
            lbid = MapId(lbid);
        }
        mDefaultLbUi = lbid;
    ...
    public void LoadFriends(ILocalUser user, Action<bool> callback) {
        GPLogger.w("PlayGamesPlatform.LoadFriends not implemented.");
        if (callback != null) {
            callback.Invoke(false);
        }
    }
    ...
    public void LoadState(int slot, OnStateLoadedListener listener) {
        if (!IsAuthenticated()) {
            GPLogger.e("LoadState can only be called after authentication.");
            if (listener != null) {
    ...
    public void UpdateState(int slot, byte[] data, OnStateLoadedListener listener) {
        if (!IsAuthenticated()) {
            GPLogger.e("UpdateState can only be called after authentication.");
            if (listener != null) {
    ...
    private string MapId(string id) {
        ...
        if (mIdMap.ContainsKey(id)) {
            ...
            GPLogger.d("Mapping alias " + id + " to ID " + result);
            return result;
        }
        return id;
    }
