...
public class NativeRealtimeMultiplayerClient : IRealTimeMultiplayerClient {
    ...
    public void CreateQuickGame(uint minOpponents, uint maxOpponents, uint variant,
        ...
        lock (mSessionLock) {
            ...
            if (mCurrentSession.IsActive()) {
                GPLogger.e("Received attempt to create a new room without cleaning up the old one.");
            ...
            mRealtimeManager.ShowPlayerSelectUI(minOpponents, maxOppponents, true,
                response => {
                    if (response.Status() != Status.UIStatus.VALID) {
                        GPLogger.d("User did not complete invitation screen.");
                        newRoom.LeaveRoom();
                        return;
                    }
    ...
    public void AcceptFromInbox(RealTimeMultiplayerListener listener) {
        lock (mSessionLock) {
            var newRoom = new RoomSession(mRealtimeManager, listener);
            if (mCurrentSession.IsActive()) {
                GPLogger.e("Received attempt to accept invitation without cleaning up " +
                    "active session.");
                newRoom.LeaveRoom();
                return;
            }
            ...
            mRealtimeManager.ShowRoomInboxUI(
                response => {
                    ...
                    using (var helper = HelperForSession(newRoom)) {
                        GPLogger.d("About to accept invitation " + invitation.Id());
                        newRoom.StartRoomCreation(mNativeClient.GetUserId(),
        ...
        lock (mSessionLock) {
            ...
            mRealtimeManager.FetchInvitations(response => {
                if (!response.RequestSucceeded()) {
                    GPLogger.e("Couldn't load invitations.");
                    newRoom.LeaveRoom();
                    return;
                }
                ...
                GPLogger.e("Room creation failed since we could not find invitation with ID "
                    + invitationId);
                newRoom.LeaveRoom();
            });
        }
    ...
    private class RoomSession {
        ...
        internal void EnterState(State handler) {
            lock (mLifecycleLock) {
                ...
                GPLogger.d("Entering state: " + handler.GetType().Name);
        ...
        internal void StartRoomCreation(string currentPlayerId, Action createRoom) {
            lock (mLifecycleLock) {
                if (!mStillPreRoomCreation) {
                    GPLogger.e("Room creation started more than once, this shouldn't happen!");
                    return;
                }

                if (!mState.IsActive()) {
                    GPLogger.w("Received an attempt to create a room after the session was already " +
                        "torn down!");
                    return;
                }
    ...
    internal abstract class State {
        internal virtual void HandleRoomResponse(RealtimeManager.RealTimeRoomResponse response) {
            GPLogger.d(this.GetType().Name + ".HandleRoomResponse: Defaulting to no-op.");
        }

        internal virtual bool IsActive() {
            GPLogger.d(this.GetType().Name + ".IsNonPreemptable: Is preemptable by default.");
            return true;
        }

        internal virtual void LeaveRoom() {
            GPLogger.d(this.GetType().Name + ".LeaveRoom: Defaulting to no-op.");
        }

        internal virtual void OnStateEntered() {
            GPLogger.d(this.GetType().Name + ".OnStateEntered: Defaulting to no-op.");
        }

        internal virtual void OnRoomStatusChanged(NativeRealTimeRoom room) {
            GPLogger.d(this.GetType().Name + ".OnRoomStatusChanged: Defaulting to no-op.");
        }

        internal virtual void OnConnectedSetChanged(NativeRealTimeRoom room) {
            GPLogger.d(this.GetType().Name + ".OnConnectedSetChanged: Defaulting to no-op.");
        }

        internal virtual void OnParticipantStatusChanged(NativeRealTimeRoom room,
            MultiplayerParticipant participant) {
            GPLogger.d(this.GetType().Name + ".OnParticipantStatusChanged: Defaulting to no-op.");
        }

        internal virtual void OnDataReceived(NativeRealTimeRoom room, MultiplayerParticipant sender,
            byte[] data, bool isReliable) {
            GPLogger.d(this.GetType().Name + ".OnDataReceived: Defaulting to no-op.");
        }

        internal virtual void SendToSpecificRecipient(
            string recipientId, byte[] data, int offset, int length, bool isReliable) {
            GPLogger.d(this.GetType().Name + ".SendToSpecificRecipient: Defaulting to no-op.");
        }

        internal virtual void SendToAll(byte[] data, int offset, int length, bool isReliable) {
            GPLogger.d(this.GetType().Name + ".SendToApp: Defaulting to no-op.");
        }

        internal virtual List<Participant> GetConnectedParticipants() {
            GPLogger.d(this.GetType().Name + ".GetConnectedParticipants: Returning empty connected" +
                " participants");
            return new List<Participant>();
        }

        internal virtual Participant GetSelf() {
            GPLogger.d(this.GetType().Name + ".GetSelf: Returning null self.");
            return null;
        }

        internal virtual Participant GetParticipant(string participantId) {
            GPLogger.d(this.GetType().Name + ".GetSelf: Returning null participant.");
            return null;
        }

        internal virtual bool IsRoomConnected() {
            GPLogger.d(this.GetType().Name + ".IsRoomConnected: Returning room not connected.");
            return false;
        }
    }
    ...
    private abstract class MessagingEnabledState : State {
        ...
        internal override void SendToSpecificRecipient(
            ...
            if (!mNativeParticipants.ContainsKey(recipientId)) {
                GPLogger.e("Attempted to send message to unknown participant " + recipientId);
                return;
            }
    ...
    class BeforeRoomCreateStartedState : State {
        ...
        internal override void LeaveRoom() {
            GPLogger.d("Session was torn down before room was created.");
    ...
    class RoomCreationPendingState : State {
        ...
        internal override void LeaveRoom() {
            GPLogger.d("Received request to leave room during room creation, aborting creation.");
    ...
    class ConnectingState : MessagingEnabledState {
        ...
        internal override void HandleConnectedSetChanged(NativeRealTimeRoom room) {
            ...
            if (mConnectedParticipants.Equals(newConnectedSet)) {
                GPLogger.w("Received connected set callback with unchanged connected set!");
                return;
            }
            ...
            if (noLongerConnected.Any()) {
                GPLogger.e("Participants disconnected during room setup, failing. " +
            ...
            GPLogger.d("New participants connected: " +
            ...
            if (newConnectedSet.Count() == room.ParticipantCount()) {
                GPLogger.d("Fully connected! Transitioning to active state.");
        ...
        internal override void HandleParticipantStatusChanged(NativeRealTimeRoom room,
            ...
            GPLogger.e(string.Format("Participant {0} changed to status {1}, room will never be" +
    ...
    class ActiveState : MessagingEnabledState {
        ...
        internal override void OnStateEntered() {
            if (GetSelf() == null) {
                GPLogger.e("Room reached active state with unknown participant for the player");
                LeaveRoom();
            }
        }
        ...
        internal override Participant GetParticipant(string participantId) {
            if (!mParticipants.ContainsKey(participantId)) {
                GPLogger.e("Attempted to retrieve unknown participant " + participantId);
                return null;
            }
        ...
        internal override void HandleConnectedSetChanged(NativeRealTimeRoom room) {
            ...
            GPLogger.d("Updated participant statuses: " +
            ...
            if (newlyLeft.Contains(GetSelf().ParticipantId)) {
                GPLogger.w("Player was disconnected from the multiplayer session.");
            }
