...
public class NativeClient : IPlayGamesClient {
    ...
    private static void InvokeCallbackOnGameThread(Action<bool> callback, bool success) {
        ...
        PlayGamesHelperObject.RunOnGameThread(() => {
            GPLogger.d("Invoking user callback on game thread");
            callback(success);
        });
    }
    ...
    internal void HandleInvitation(Types.MultiplayerEvent eventType, string invitationId,
        ...
        if (currentHandler == null) {
            GPLogger.d("Received " + eventType + " for invitation "
            + invitationId + " but no handler was registered.");
            return;
        }

        if (eventType == Types.MultiplayerEvent.REMOVED) {
            GPLogger.d("Ignoring REMOVED for invitation " + invitationId);
            return;
        }
    ...
    private void PopulateAchievements(uint authGeneration,
        ...
        if (authGeneration != mAuthGeneration) {
            GPLogger.d("Received achievement callback after signout occurred, ignoring");
            return;
        }

        GPLogger.d("Populating Achievements");
        lock (AuthStateLock) {
            if (response.Status() != Status.ResponseStatus.VALID &&
                response.Status() != Status.ResponseStatus.VALID_BUT_STALE) {
                GPLogger.e("Error retrieving achievements - check the log for more information. " +
                    "Failing signin.");
        ...
        GPLogger.d("Maybe finish for Achievements");
    ...
    void MaybeFinishAuthentication() {
        ...
        lock (AuthStateLock) {
            ...
            if (mUser == null || mAchievements == null) {
                GPLogger.d("Auth not finished. User=" + mUser + " achievements=" + mAchievements);
                return;
            }

            GPLogger.d("Auth finished. Proceeding.");
        ...
        if (localCallbacks != null) {
            GPLogger.d("Invoking Callbacks: " + localCallbacks);
    ...
    void PopulateUser(uint authGeneration, PlayerManager.FetchSelfResponse response) {
        GPLogger.d("Populating User");

        if (authGeneration != mAuthGeneration) {
            GPLogger.d("Received user callback after signout occurred, ignoring");
            return;
        }
        ...
        lock (AuthStateLock) {
            if (response.Status() != Status.ResponseStatus.VALID &&
                response.Status() != Status.ResponseStatus.VALID_BUT_STALE) {
                GPLogger.e("Error retrieving user, signing out");
        ...
        GPLogger.d("Found User: " + mUser);
        GPLogger.d("Maybe finish for User");
        MaybeFinishAuthentication();
    }

    private void HandleAuthTransition(Types.AuthOperation operation, Status.AuthStatus status) {
        GPLogger.d("Starting Auth Transition. Op: " + operation + " status: " + status);
        lock (AuthStateLock) {
            switch (operation) {
                ...
                case Types.AuthOperation.SIGN_OUT:
                    ...
                    break;
                default:
                    GPLogger.e("Unknown AuthOperation " + operation);
                    break;
            }
        }
    }
    ...
    private void UpdateAchievement(string updateType, string achId, Action<bool> callback,
        ...
        if (achievement == null) {
            GPLogger.d("Could not " + updateType + ", no achievement with ID " + achId);
            callback(false);
            return;
        }

        if (alreadyDone(achievement)) {
            GPLogger.d("Did not need to perform " + updateType + ": " + "on achievement " + achId);
            callback(true);
            return;
        }

        GPLogger.d("Performing " + updateType + " on " + achId);
    ...
    public void IncrementAchievement(string achId, int steps, Action<bool> callback) {
        ...
        var achievement = GetAchievement(achId);
        if (achievement == null) {
            GPLogger.e("Could not increment, no achievement with ID " + achId);
            callback(false);
            return;
        }

        if (!achievement.IsIncremental) {
            GPLogger.e("Could not increment, achievement with ID " + achId + " was not incremental");
            callback(false);
            return;
        }

        if (steps < 0) {
            GPLogger.e("Attempted to increment by negative steps");
            callback(false);
            return;
        }
    ...
    public void LoadState(int slot, OnStateLoadedListener listener) {
        ...
        lock (GameServicesLock) {
            if (mAuthState != AuthState.Authenticated) {
                GPLogger.e("You can only call LoadState after the user has successfully logged in");
    ...
    public void UpdateState(int slot, byte[] data, OnStateLoadedListener listener) {
        ...
        lock (GameServicesLock) {
            if (mAuthState != AuthState.Authenticated) {
                GPLogger.e("You can only call UpdateState after the user has successfully logged in");
