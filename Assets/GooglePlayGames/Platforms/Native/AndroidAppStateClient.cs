...
internal class AndroidAppStateClient : AppStateClient {
    ...
    private class OnStateResultProxy : AndroidJavaProxy {
        ...
        private void OnStateConflict(int stateKey, string resolvedVersion,
			...
			GPLogger.d("OnStateResultProxy.onStateConflict called, stateKey=" + stateKey +
            ...
            if (mListener != null) {
				GPLogger.d("OnStateResultProxy.onStateConflict invoking conflict callback.");
				...
				GPLogger.w("No conflict callback specified! Cannot resolve cloud save conflict.");
            }
        }

        private void ResolveState(int slot, string resolvedVersion, byte[] resolvedData,
            OnStateLoadedListener listener) {
			GPLogger.d(string.Format("AndroidClient.ResolveState, slot={0}, ver={1}, " +
        ...
        private void OnStateLoaded(int statusCode, int stateKey, byte[] localData) {
			GPLogger.d("OnStateResultProxy.onStateLoaded called, status " + statusCode +
                ", stateKey=" + stateKey);
            debugLogData("localData", localData);
            ...
            switch (statusCode) {
                case STATUS_OK:
					GPLogger.d("Status is OK, so success.");
                    success = true;
                    break;
                case STATUS_NO_DATA:
					GPLogger.d("Status is NO DATA (no network?), so it's a failure.");
                    success = false;
                    localData = null;
                    break;
                case STATUS_STALE_DATA:
					GPLogger.d("Status is STALE DATA, so considering as success.");
                    success = true;
                    break;
                case STATUS_KEY_NOT_FOUND:
					GPLogger.d("Status is KEY NOT FOUND, which is a success, but with no data.");
                    success = true;
                    localData = null;
                    break;
                default:
					GPLogger.e("Cloud load failed with status code " + statusCode);
                    success = false;
                    localData = null;
                    break;
            }
            ...
            if (mListener != null) {
				GPLogger.d("OnStateResultProxy.onStateLoaded invoking load callback.");
                PlayGamesHelperObject.RunOnGameThread(() => {
                    mListener.OnStateLoaded(success, stateKey, localData);
                });
            } else {
				GPLogger.w("No load callback specified!");
            }
        }

        private void debugLogData(string tag, byte[] data) {
			GPLogger.d("   " + tag + ": " + GPLogger.describe(data));
        }

        public void onResult(AndroidJavaObject result) {
			GPLogger.d("OnStateResultProxy.onResult, result=" + result);

            int statusCode = GetStatusCode(result);
			GPLogger.d("OnStateResultProxy: status code is " + statusCode);

            if (result == null) {
				GPLogger.e("OnStateResultProxy: result is null.");
                return;
            }

            GPLogger.d("OnstateResultProxy: retrieving result objects...");
            ...
            GPLogger.d("Got result objects.");
            GPLogger.d("loadedResult = " + loadedResult);
            GPLogger.d("conflictResult = " + conflictResult);

            if (conflictResult != null) {
                GPLogger.d("OnStateResultProxy: processing conflict.");
                ...
                GPLogger.d("OnStateResultProxy: conflict args parsed, calling.");
                OnStateConflict(stateKey, ver, localData, serverData);
            } else if (loadedResult != null) {
                GPLogger.d("OnStateResultProxy: processing normal load.");
                int stateKey = loadedResult.Call<int>("getStateKey");
                byte[] localData = ToByteArray(loadedResult.NullSafeCall("getLocalData"));
                GPLogger.d("OnStateResultProxy: loaded args parsed, calling.");
                OnStateLoaded(statusCode, stateKey, localData);
            } else {
                GPLogger.e("OnStateResultProxy: both loadedResult and conflictResult are null!");
            }
        }
    }
