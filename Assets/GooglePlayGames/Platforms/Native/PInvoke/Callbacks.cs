...
static class Callbacks {
    ...
    internal static readonly Action<CommonErrorStatus.UIStatus> NoopUICallback = status => {
        GPLogger.d("Received UI callback: " + status);
    };
    ...
    private static T IntPtrToCallback<T>(IntPtr handle, bool unpinHandle) where T : class {
        ...
        try {
            ...
            GPLogger.e("GC Handle pointed to unexpected type: " + gcHandle.Target.ToString() +
                ". Expected " +  typeof(T));
            throw e;
        } finally {
            if (unpinHandle) {
    ...
    internal static void InternalShowUICallback(CommonErrorStatus.UIStatus status, IntPtr data) {
        GPLogger.d("Showing UI Internal callback: " + status);
        var callback = IntPtrToTempCallback<Action<CommonErrorStatus.UIStatus>>(data);

        try {
            callback(status);
        } catch (Exception e) {
            GPLogger.e("Error encountered executing InternalShowAllUICallback. " +
    ...
    internal static void PerformInternalCallback(string callbackName, Type callbackType,
        ...
        GPLogger.d("Entering internal callback for " + callbackName);
        ...
        try {
            ...
            GPLogger.e("Error encountered executing " + callbackName + ". " +
            "Smothering to avoid passing exception into Native: " + e);
        }
    }
