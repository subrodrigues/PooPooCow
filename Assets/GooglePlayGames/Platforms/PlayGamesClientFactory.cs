...
internal class PlayGamesClientFactory {
    internal static IPlayGamesClient GetPlatformPlayGamesClient(
        ...
        if (Application.isEditor) {
            GPLogger.d("Creating IPlayGamesClient in editor, using DummyClient.");
            return new GooglePlayGames.BasicApi.DummyClient();
        }
#if (UNITY_ANDROID || (UNITY_IPHONE && !NO_GPGS))
        GPLogger.d("Creating real IPlayGamesClient");
        return new GooglePlayGames.Native.NativeClient(config);
#else
            GPLogger.d("Cannot create IPlayGamesClient for unknown platform, returning DummyClient");
            return new GooglePlayGames.BasicApi.DummyClient();
#endif
    }
}
